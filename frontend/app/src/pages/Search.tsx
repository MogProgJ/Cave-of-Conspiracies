...
import { Search as SearchIcon, Clock, TrendingUp, X, ChevronRight, Activity, Users, Radio } from "lucide-react";
import { Link } from "react-router-dom";
import { searchContent, SearchResponse } from "../lib/api";
...
export default function Search() {
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      const res = await searchContent({ q, limit: 20 });
      if (res.ok) {
        setResults(res.data);
        setError(null);
      } else {
  ...
  const filteredThreads = results?.threads ?? [];
      ...
      <div className="min-h-[500px]">
        {!query ? (
            ...
            <div className="glass-dark rounded-2xl p-8 border-white/5 space-y-6">
              ...
              <ul className="space-y-4">
                {[
                  ...
                  <li key={i} onClick={() => setQuery(t)} className="flex items-center justify-between group cursor-pointer p-2 -mx-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                  ...
                  <ul className="space-y-4">
                    {recentSearches.map((t, i) => (
                      <li key={i} onClick={() => setQuery(t)} className="flex items-center justify-between group cursor-pointer p-2 -mx-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                        ...
                        <X onClick={(e) => handleRemoveSearch(e, t)} className="w-3 h-3 opacity-30 hover:opacity-100 hover:text-red-400" />
                      </li>
                    ))}
                  </ul>
                  <button onClick={handleClearAll} className="type-ui text-xs text-white/30 hover:text-white transition-colors underline underline-offset-4 pt-2">
                    Clear History
                  </button>
                </>
              ) : (
                <div className="py-8 text-left text-white/30 type-ui text-sm italic">No recent expeditions.</div>
              )}
            </div>
          </motion.div>
        ) : (
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-24 text-center type-ui text-sm text-white/50">
                Scanning backend archives...
              </motion.div>
            ) : error ? (
              <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-24 text-center type-ui text-sm text-red-300">
                Search failed: {error}
              </motion.div>
            ) : hasResults ? (
              <motion.div key="results" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {filteredCommunities.map((c) => (
                        <Link to={`/community/${c.slug}`} key={c.slug} className="glass-dark p-5 rounded-2xl border-white/5 hover:bg-white/[0.02] transition-colors group flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="type-ui text-xs text-white/40 font-semibold">{c.posts_7d} posts in 7d</p>
                          </div>
                        </Link>
                      ))}
                    </div>
                    ...
                    <div className="space-y-3">
                      {filteredThreads.map((t) => (
                        <Link to={`/thread/${t.id}`} key={t.id} className="block glass-dark p-6 rounded-2xl border-white/5 hover:bg-white/[0.02] transition-colors group">
                          <div className="type-ui text-[10px] font-bold uppercase tracking-widest text-violet-400 opacity-80 mb-2">{t.community?.name ?? 'General'} // NODE</div>
                          <h5 className="type-title text-lg mb-2 group-hover:text-violet-300 transition-colors">{t.title}</h5>
                          <p className="type-ui text-sm text-white/60 line-clamp-2">{t.body_preview}</p>
                        </Link>
                      ))}
                    </div>
                  </div>
              ...
              <motion.div key="no-results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-24 text-center space-y-6 opacity-60">
                ...
                <button onClick={() => setQuery("")} className="type-ui text-sm font-semibold hover:text-white transition-colors underline underline-offset-4 pt-2">
                  Restart Interceptor
                </button>
              </motion.div>
            )}
